...
import { paysUpdateSchema } from "@/lib/validation/schemas";
...
export const PATCH = handler<Ctx>(async (req, ctx) => {
  ...
  const id = await parseId(ctx);
  const body = await validateBody(req, paysUpdateSchema);

  try {
    return jsonOk(await service.update(id, body.nom_pays));
  } catch (e: any) {
    if (e?.code === "ER_DUP_ENTRY") return jsonError(409, "CONFLICT", "nom_pays already exists");
    throw e;
  }
});
