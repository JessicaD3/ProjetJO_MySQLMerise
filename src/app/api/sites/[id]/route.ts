...
import { siteUpdateSchema } from "@/lib/validation/schemas";
...
export const PATCH = handler<Ctx>(async (req, ctx) => {
  ...
  const id = await parseId(ctx);
  const body = await validateBody(req, siteUpdateSchema);

  try {
    return jsonOk(await service.update(id, body.nom_site, body.capacite));
  } catch (e: any) {
